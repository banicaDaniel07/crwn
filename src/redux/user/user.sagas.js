...
export function* getSnapshopFormAuth(userAuth, additionalData) {
    try {
        const userRef = yield call(createUserProfileDocument, userAuth, additionalData);
        const userSnappshot = yield userRef.get();

        yield put(signInSuccess({ id: userSnappshot.id, ...userSnappshot.data() }));
    } catch (error) {
...
export function* signInWithGoogle() {
    try {
        ...
        yield getSnapshopFormAuth(user);
    } catch(error) {
        yield put(signInFailure(error));
    }
};
...
export function* isUserAuthenticated(){
    try {
        ...
        if(!userAuth) return;
        yield getSnapshopFormAuth(userAuth);
    } catch (error) {
        yield put(signInFailure(error));
    }
}
...
export function* signInAfterSignUp({ payload: {user, aditionalData}}) {
    yield getSnapshopFormAuth( user, aditionalData);
}
